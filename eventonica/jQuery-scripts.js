$(document).ready(() => {
    ...
    website.addUser('lizandry', 'liz', 94608, [00001]);
    ...
    $('#push-event-button').on('click', function(event) {
        ...
        let event_ = website.events.find(event => event.id === eventID);
        // Update matching user to have event (user.saveEvent(event))
        website.saveUserEvent(user, event_)
        console.log(website.users[0].savedEvents);
        // $('#pull-user').saveUserEvent('#pull-event')
    });
    //this is close, it's just not adding events to the user
